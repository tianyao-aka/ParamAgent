    ...
    for i, item in enumerate_resume(dataset, log_path):
        ...
        while cur_pass < pass_at_k and not is_solved:
            ...
            while cur_func_impl is None:
                if use_mistakes:
                    if mistake_json_file is not None:
                        refined_insights = mistake_json_file[i]['refined_insights']
                    elif pitfall_agent is not None:
                        refined_insights = pitfall_agent.generate(item["prompt"])
                    else:
                        # use gpt-4o-mini to generate pitfalls
                        refined_insights = gen.generate_pre_insights(item["prompt"])
                    item['refined_insights'] = refined_insights
                    start_time = time()
                    cur_func_impl = gen.func_impl(item["prompt"], model, "simple",mistake_insights=refined_insights,temperature=0.2)
                else:
            ...
            while cur_iter < max_iters:
                # get self-reflection
                refined_insights = pitfall_agent.generate(item["prompt"],temperature=0.8)
                reflection = gen.self_reflection_parametric(
                    cur_func_impl, cur_feedback,refined_insights, model)
                ...
                while new_func_impl is None:
                    new_func_impl = gen.func_impl(
                        ...
                        mistake_insights=refined_insights,   # no mistake insights for reflexion
                        temperature=0.2
                    )
        ...
        # except:
        #     print (colored(f"Error processing item {i}", 'red'))
        #     continue
        end_time = time()
