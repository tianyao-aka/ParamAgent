#!/usr/bin/env python
"""
Multi-GPU inference for the LoRA-fine-tuned Meta-Llama-3.1-8B coding model.
...
Example (4 GPUs data-parallel):
  CUDA_VISIBLE_DEVICES=4,5,6,7 python code/LoRA_Llama3_Code_multigpu_inference.py \
    --base_model meta-llama/Meta-Llama-3.1-8B-Instruct \
    --lora_path ./lora-llama3-8b-code \
    --input_jsonl benchmarks/humaneval_full.jsonl \
    --output_jsonl generated_reflections/code/humaneval_pitfalls_llama3_8b.jsonl \
    ...
    with jsonlines.open(output_path, mode=file_mode) as writer:
        for chunk in tqdm(
            ...
            for item, gens in zip(chunk, grouped):
                ...
                if args.output_key == "high_temp_pitfall" and "pitfall" not in item:
                    item["pitfall"] = gens[0] if gens else ""
                writer.write(item)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("LoRA Llama3-8B multi-GPU inference")
