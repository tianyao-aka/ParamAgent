...
import argparse
import sys
from simple_QA import run_simple
# from test_acc import run_test_acc  # Commented out - unused import
from utils import read_jsonl, read_jsonl_gz, read_jsonl_map
import json
# from dot_QA import run_dot
# from dot_QA_parametric import run_dot
from dot_QA_parametric_with_bank import run_dot
# from dot_bank import run_dot_bank
try:
    from LoRA_Llama3_QA_inference import QADecomposer
except ModuleNotFoundError:
    QADecomposer = None  # Module not available
import torch
import gc
...
""")
    ...
    if len(args.insight_json_path)>3:
        ...
        if 'high_temp_insight' not in insight_json_file[0]:
            if QADecomposer is not None:
                QA_agent = QADecomposer(
                    adapter_dir="./LoRA/qa/",
                ...
                print("WARNING: QADecomposer not available, setting to None")
                QA_agent = None
        else:
            QA_agent = None
    ...
    if args.strategy != 'simple': 
        ...
        print ('error! not running simple strategy')    
        pass
