...
""")
    ...
    if args.use_expert_module:
        if len(args.insight_json_path)>3:
            ...
            if QADecomposer is None:
                raise ModuleNotFoundError(
                    "LoRA_Llama3_QA_inference is not available. "
                    "Pass --insight_json_path or use mainQA_parametric.py with the available inference path."
                )
