        ...
        while cur_pass < pass_at_k and not is_solved:
            ...
            while init_iter < 5:
                ...
                while cur_func_impl is None:
                    cur_func_impl = gen.func_impl(item["question"],item["context"], model, "simple",temperature=0.2,question_decomposition= refined_insights)
                    fail_cnt += 1
                    if fail_cnt > 3:
                        break
            ...
            while cur_iter < max_iters:
                ...
                div_reflections = gen.self_reflection_diverse_parametric(
                    item["question"], cur_func_impl,item["context"], cur_feedback, model,diverse_reflections,refined_insights).split('\n\n')
                ...
                while new_func_impl is None:
                    new_func_impl = gen.func_impl(
                        ...
                        self_reflection=reflection, temperature = 0.2, question_decomposition=refined_insights
                    )
                    fail_cnt += 1
                    if fail_cnt > 3:
                        break
            ...
            cur_pass += 1
        # except:
        #     print (colored(f"Error in item {i}", 'red'))
        #     continue
        end_time = time()
