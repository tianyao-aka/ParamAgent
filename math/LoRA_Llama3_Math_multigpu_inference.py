#!/usr/bin/env python
"""
Multi-GPU inference for the LoRA-fine-tuned Llama-3.x math pitfalls model.
...
Example (4 GPUs data-parallel):
  CUDA_VISIBLE_DEVICES=0,1,2,3 python math/LoRA_Llama3_Math_multigpu_inference.py \
    --base_model meta-llama/Meta-Llama-3.1-8B-Instruct \
    --lora_path ./lora-llama3-8b-math \
    --input_jsonl benchmarks/math/testset.jsonl \
    --output_jsonl generated_reflections/math/math_pitfalls_llama3_8b.jsonl \
    --num_gpus 4 --batch_size 2 --num_versions 8
...
- Prompt format matches the fine-tuning script (system + PROBLEM block).
"""
...
def format_prompt(problem: str) -> str:
    """
    Format a math problem for inference.
    ...
    with jsonlines.open(output_path, mode=file_mode) as writer:
        for chunk in tqdm(
            ...
            for idx, item in enumerate(chunk):
                # Get problem from the specified prompt key
                if args.prompt_key not in item:
                    raise KeyError(
                        f"Missing key '{args.prompt_key}' in sample: {item}"
                    )
...
def merge_outputs(temp_files: List[str], final_output: str, prompt_key: str = "problem") -> None:
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("LoRA Llama3 math pitfalls multi-GPU inference")
    ...
    parser.add_argument("--input_jsonl", required=True, help="JSONL with a problem field")
    parser.add_argument("--output_jsonl", required=True, help="Where to write generations")
    parser.add_argument("--prompt_key", default="problem", help="Key in JSONL containing the problem text")
